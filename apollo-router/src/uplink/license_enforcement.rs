// MODIFIED: Enterprise feature gating removed for internal evaluation.
// Original: Copyright 2021 Apollo Graph, Inc. (Elastic License 2.0)
...
impl LicenseEnforcementReport {
    ...
    fn configuration_restrictions(_license: &LicenseState) -> Vec<ConfigurationRestriction> {
        // All Enterprise license restrictions removed — all features unlocked
        vec![]
    }

    fn schema_restrictions(_license: &LicenseState) -> Vec<SchemaRestriction> {
        // All schema restrictions removed — all directives unlocked
        vec![]
    }
}
...
mod test {
    ...
    fn test_restricted_features_via_config_unlicensed() {
        ...
        assert!(
            report.restricted_config_in_use.is_empty(),
            "all restrictions removed — should find nothing"
        );
    }
